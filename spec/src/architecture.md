...
| ID         | Requirement |
| ---------- | ----------- |
| TH-PKG-001 | Private `@lapismd/terminal-host` MUST live at the repository root, expose `build`, `check`, `test`, and a `lapis-terminal-host` CLI. It MUST own PTY spawn and standalone transport. Consumer plugins MUST NOT depend on it at runtime. |
