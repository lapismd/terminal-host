...
| TH-PROTO-008 | When attaching session planes, the web client MUST open `/terminal/control`, apply a non-empty restore snapshot, then open `/terminal/io`. It MUST NOT emit restore bytes and live I/O for the same attach overlap. |
