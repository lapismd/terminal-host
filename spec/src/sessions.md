...
| TH-SESS-003 | The session service MUST support create, write, resize, stop, list, attach, and a bounded restore snapshot of recent output. Live output and the public restore snapshot MUST remain raw `Uint8Array` bytes; protocol text projection MAY be separate. Closing a session MUST close the PTY and emit at most one exit event. |
