...
| ID           | Requirement |
| ------------ | ----------- |
| TH-SESS-001 | A new session MUST spawn a login-interactive host shell (`$SHELL -il`, or Windows `COMSPEC` / PowerShell) through `node-pty`. It MUST inherit the host process environment, set `TERM=xterm-256color`, and accept an optional absolute `shell` on create. |
